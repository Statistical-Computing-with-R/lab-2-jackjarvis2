...
the data types are some numeric and some character. 
<!-- You can either write code here or in the code chunk for #1, but I **must** see the code you used to address this question!  -->
...
ggplot(data = surveys, mapping = aes(x = weight, y = hindfoot_length)) + 
  ...
  facet_wrap(~ species) + 
  labs(title = "Weight vs Hindfoot Length of Rodents", 
...
ggplot(data = surveys, aes(x = species, y = weight)) + 
  ...
  geom_jitter(color = "orange3", alpha = 0.5) +
...
```{r}
#| label: anova
anova_result <- aov(weight ~ species, data = surveys)
...
Since we see in our conlcusion that we have a high f value and a p-value that is less than 0.05 we can conclude that the we reject the null hypothesis and that least one rodent species has a different population mean weight than the others. 
